...
struct
	...
	let all : t list = List.concat [allSpades;allHearts;allDiamonds;allClubs]
...
let newDeck = 
	let rand c1 c2 =  Random.self_init () ; if (Random.bool ()) then (-1) else 1 in
	List.stable_sort rand Card.all

let rec toStringList (lst:t) = match lst with
	| head::tail -> (Card.toString head) :: toStringList tail
	| _ -> []

let rec toStringListVerbose (lst:t) = match lst with
