...
int			bdin_detected(t_sh *sh)
{
	...
	if (ft_strequ("encrypt", sh->cmd[0]))
		return (bdin_crypt(sh, 'e'));
	if (ft_strequ("decrypt", sh->cmd[0]))
		return (bdin_crypt(sh, 'd'));
	if (ft_strequ("sort", sh->cmd[0]))
		return (bdin_sort(sh));
	if (ft_strequ("rand", sh->cmd[0]))
		return (bdin_rand(sh));
	return (0);
}
